...
EOFWBK
        ...
        # The Fedora WebKit build embeds a hardcoded relative helper path
        # "././/libexec/webkit2gtk-4.1" in the library.  At AppImage runtime
        # the helpers live under usr/libexec/ — create a root-level symlink
        # so the relative path resolves.  Same for the injected-bundle .so
        # (hardcoded at "././/lib64/webkit2gtk-4.1/injected-bundle/").
        ln -sfn usr/libexec "$APPDIR/libexec"
        ln -sfn usr/lib64   "$APPDIR/lib64"
        # Copy the injected-bundle .so into the expected lib64 subtree so
        # the symlink above resolves to a real file at runtime.
        for _ib_dir in /usr/lib64/webkit2gtk-4.1/injected-bundle \
                       /usr/lib/x86_64-linux-gnu/webkit2gtk-4.1/injected-bundle; do
            if [ -f "$_ib_dir/libwebkit2gtkinjectedbundle.so" ]; then
                mkdir -p "$APPDIR/usr/lib64/webkit2gtk-4.1/injected-bundle"
                cp -a "$_ib_dir/libwebkit2gtkinjectedbundle.so" \
...
# --- Post-build: udev rules for device passthrough ---
echo ""
echo "================================================"
echo "DEVICE ACCESS (udev rules)"
echo "================================================"
echo "To allow non-root USB access for Arctic Fox / Nuvoton / STM devices,"
echo "copy and paste the following command into your terminal:"
echo ""
echo "  sudo tee /etc/udev/rules.d/99-cloudy-af.rules > /dev/null <<'UDEV'"
echo 'ACTION=="add", SUBSYSTEM=="usb", ATTR{idVendor}=="0416", ATTR{idProduct}=="5020", MODE="0666", GROUP="plugdev"'
echo 'ACTION=="add", SUBSYSTEM=="usb", ATTR{idVendor}=="0483", ATTR{idProduct}=="5750", MODE="0666", GROUP="plugdev"'
echo "UDEV"

...
fn test_flash_uhid_double() {
    //! End-to-end flasher gate against the /dev/uhid LDROM double
    //! (scripts/uhid_ldrom.py, validation route §5). Requires the uhid udev
    //! rule and the REAL Pico 25 (0416:5020) UNPLUGGED: the gate asserts the
    //! double is the only matching device so it can never touch hardware.
    //! (The Nu-Link ICE 0416:511C is NOT in SUPPORTED_DEVICES and does not
    //! trip this gate.)
    if std::fs::OpenOptions::new()
        ...
        .is_err()
    {
        eprintln!("no writable /dev/uhid (udev rule missing?); skipping");
        return;
    }
    let script = concat!(env!("CARGO_MANIFEST_DIR"), "/../scripts/uhid_ldrom.py");
    let child = std::process::Command::new("python3")
        .arg(script)
        .spawn()
        .expect("spawn uhid_ldrom.py");
    let guard = scopeguard(child);
    std::thread::sleep(std::time::Duration::from_millis(800));

    let devs = flasher::list_devices().expect("list_devices failed");
    assert_eq!(
        devs.len(),
        1,
        "uhid gate: exactly one device (the double) may be present, found {devs:?} — unplug the real device"
    );
    assert!(
        devs[0].serial.ends_with("-uhid"),
        "uhid gate: device is not the double: {devs:?}"
    );

    // 1024 bytes of deterministic plaintext (minimum the size guard accepts).
    // NOT larger on purpose: the kernel's uhid output ring holds 31 events
    // and silently DROPS reports past that while the host write still
    // succeeds (uhid_queue in drivers/hid/uhid.c) — there is no NAK
    // backpressure like real USB, so a burst bigger than the ring is lost
    // before a userspace consumer can drain it. 1024 B = 16 data reports +
    // 1 command = 17 events, safely under the limit. The write-retry path
    // (50x50 ms backoff for real-hardware NAKs) cannot be exercised through
    // uhid at all, for the same reason.
    let img: Vec<u8> = (0..1024u32).map(|i| (i % 251) as u8).collect();
    let res = flasher::flash_firmware_guarded(&img, None, &|s| println!("flash: {s}"));
    drop(guard);
    res.expect("flash against uhid double failed");
}

/// Kill the double on drop, even when the test panics.
fn scopeguard(child: std::process::Child) -> impl Drop {
